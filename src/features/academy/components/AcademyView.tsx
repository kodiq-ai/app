// ── Academy View ─────────────────────────────────────────────────────────────
// Main container for Academy mode. Shows AuthPrompt or WebView.

import { useCallback, useEffect, useRef } from "react";
import { ArrowLeft, RotateCw } from "lucide-react";
import { useAppStore } from "@/lib/store";
import { academy } from "@shared/lib/tauri";
import { ACADEMY_URL, SUPABASE_STORAGE_KEY } from "@shared/lib/constants";
...
export function AcademyView() {
  ...
  if (!isAuthenticated) {
    return <AuthPrompt description={t("academyNotAuthenticated")} />;
  }
  ...
  return (
    <div className="flex flex-1 flex-col overflow-hidden">
      ...
      <div className="flex h-9 shrink-0 items-center gap-1 border-b border-white/[0.06] px-2">
        ...
        <span className="text-k-text-tertiary ml-2 truncate text-xs">{ACADEMY_URL}</span>
      </div>

      {/* WebView container — Rust child webview renders here */}
      <div ref={containerRef} className="relative flex-1" data-academy-container />
    </div>
  );
}
